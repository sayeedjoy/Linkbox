...
export async function PATCH(
  ...
  if (!id)
    return NextResponse.json({ error: "Group id required" }, { status: 400, headers: corsHeaders(request) });
  ...
  if (!group)
    return NextResponse.json({ error: "Group not found" }, { status: 404, headers: corsHeaders(request) });

  return NextResponse.json(
    { id: group.id, name: group.name, color: group.color, order: group.order, _count: group._count },
